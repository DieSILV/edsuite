...
class _VaultsScreenState extends State<VaultsScreen> {
  ...
  Future<void> fetchBovedas() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          bovedas = data['bovedas'] ?? [];
        });
      }
    } catch (e) {
      mostrarAlerta('Error', 'No se pudieron cargar las bóvedas.');
    }
    setState(() => isLoading = false);
  }

  Future<void> registrarBoveda() async {
    ...
    try {
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        mostrarAlerta('Error', 'No se pudo registrar la bóveda.');
      }
    } catch (_) {
      mostrarAlerta('Error', 'Error al registrar la bóveda.');
    }
  }
  ...
  Widget _buildBovedasList() {
    return Column(
      children: [
        Expanded(
          child: bovedas.isEmpty
              ? const Center(
                  child: Text(
                    'No hay bóvedas registradas.',
                    style: TextStyle(color: Colors.black54),
                  ),
                )
              : ListView.builder(
                  itemCount: bovedas.length,
                  itemBuilder: (_, i) {
                    final item = bovedas[i];
